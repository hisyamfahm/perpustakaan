        				...
        				<div class="panel-body">
                            <form action="{{ url('penelitian') }}" method="get">
                                <div class="input-group">
                                    <input class="form-control" name="cari" type="text" placeholder="Tulis Kata Kunci Pencarian">
        					...
        					<table class="table table-hover">
        						<thead>
                                    ...
                                    <th>
        								<div class="btn-group">
        									<button type="button" class="btn dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><a>Bidang<span class="caret"></span></a>
        									</button>
        									<ul class="dropdown-menu" role="menu">
            									@foreach($bidang as $bi)
            										<li><a href="{{route('bidangpenelitian',$bi->id)}}">{{$bi->Nama}}
                                    ...
                                    @foreach($data_penelitian as $pe)
                                        <tr>
                                            ...
                                            @if(Auth::check() && Auth::user()->role =='admin')
                                                ...
                                                <td><a href="{{ url('penelitian/hapus/'.$pe->id) }}" class="btn btn-danger btn-sm" onclick="return confirm('Yakin mau dihapus?')">Hapus</a></td>
                                            @endif
                                        </tr>
                                    @endforeach
                                </tbody>
        					</table>
                            <b>Halaman :</b><span class="badge">{{ $data_penelitian->currentPage() }}</span>
                            <br/>
                            <b>Jumlah Data :</b><span class="badge"> {{ $data_penelitian->total() }} </span>
                            <br/>
                            <b> Data Per Halaman : </b><span class="badge">  {{ $data_penelitian->perPage() }}</span>
                            <br/>
